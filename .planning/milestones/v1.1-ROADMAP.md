...
- [x] 06-03-PLAN.md — TYPE-01 verify-only audit (no signature edits) + `tsc --checkJs` plain-JS smoke consumer per package [TYPE-01, TYPE-03]
...
- [x] 08-03-PLAN.md — isolated `docs.yml` GitHub Pages workflow (`pages: write` + `id-token: write`; `ci.yml`/`release.yml` untouched) + one-time Pages-source human checkpoint [DOCS-06]
...
- [x] 09-01-PLAN.md — TRACER: end-to-end CEM for `forms` (analyzer config, enriched `lit-form` JSDoc, committed manifest + VS Code/JetBrains editor-data, `customElements`/`web-types` fields + `files`, shared `tools/cem-check` tag-set + `ci.yml` freshness gates, `.gitattributes` LF pins, `.vscode` dogfood) [CEM-01, CEM-02, CEM-03, CEM-04]

**Wave 2** *(blocked on 09-01)*

- [x] 09-02-PLAN.md — expand CEM to `query` (config with `src/demo.ts` exclude, enriched `lit-query-client-provider` JSDoc, manifest + editor-data, appended known-tags row) [CEM-01, CEM-02, CEM-03, CEM-04]

**Wave 3** *(blocked on 09-01, 09-02)*

- [x] 09-03-PLAN.md — expand CEM to `router` (JSDoc `@tag` `define()`-wrapper fix + enriched JSDoc on the three router elements, config with `example/**`+`my-element.ts` excludes, `node scripts/build.js` build chain, full three-package gate) [CEM-01, CEM-02, CEM-03, CEM-04]
...
- [x] 10-01-PLAN.md — TRACER: workspace wiring (npm `"*"` deps, root workspaces edit) + one end-to-end route (router shell + store seam) against the real built packages; `scripts/check-single-instance.mjs` + release exclusion (`private:true` + Changesets `ignore`) + read-only CI wiring [EXPL-01, EXPL-02, EXPL-03]
...
- [x] 11-01-PLAN.md — TRACER: scaffold the `@willramdev/devtools` leaf package (optional peers, `sideEffects:false`, externalized build, local `esm-env` dev-gate) + `attachRouterLog` over the public `router.subscribe` + Changesets `fixed`-group edit + read-only `ci.yml` leaf-rule gate [DTOOL-01, DTOOL-03, DTOOL-04]
...
- [x] 11-03-PLAN.md — `attachQueryDevtools`: lazy-mount the standalone TanStack Query Devtools panel bound to the app `QueryClient` + finalize the three-fn barrel + whole-package tree-shake/publint/attw verification [DTOOL-03, DTOOL-01]
...
- [x] 12-01-PLAN.md — `.github/dependabot.yml`: two grouped weekly `updates:` entries (npm + github-actions), majors split out, npm ignoring `lit`/`@tanstack/*`, zero PR-automation [DEPS-01, DEPS-02]
- [x] 12-02-PLAN.md — non-blocking `npm audit --audit-level=high` step in the read-only `ci.yml` gate + `actions/checkout`/`setup-node` `@v5` sweep across all four workflows (changesets SHA-pin + release auth preserved) [DEPS-02, DEPS-03]
