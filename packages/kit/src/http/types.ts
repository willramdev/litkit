...
import type { InterceptorManager } from './interceptors.ts';
...
export type HttpParams =
  | Record<string, HttpParamValue | readonly HttpParamValue[]>
  | URLSearchParams
  | string
  | [string, string][];
...
export interface HttpRequestConfig<T = any>
  extends Omit<RequestInit, 'body' | 'headers' | 'method' | 'signal' | 'window'> {
...
export interface HttpClientConfig
  extends Omit<HttpRequestConfig, 'url' | 'body' | 'schema'> {}
...
export interface HttpRequest<T = any>
  extends Omit<HttpRequestConfig<T>, 'url' | 'method' | 'headers' | 'context'> {
